...
{
#if DPCT_COMPATIBILITY_TEMP >= 800
    return sycl::half2(sycl::fmax(lhs[0], rhs[0]), sycl::fmax(lhs[1], rhs[1]));
#else
    sycl::half2 ret_val;
...
{
#if DPCT_COMPATIBILITY_TEMP >= 800
    return sycl::half2(sycl::fmin(lhs[0], rhs[0]), sycl::fmin(lhs[1], rhs[1]));
#else
    sycl::half2 ret_val;
    ret_val.x = (lhs.x < rhs.x) ? lhs.x : rhs.x;
    ret_val.y = (lhs.y < rhs.y) ? lhs.y : rhs.y;
...
DS_D_INLINE void _block(sycl::group<3>& tb, sycl::sub_group& warp_arg, T* data)
{
    ...
    /*
    DPCT1115:10: The sycl::ext::oneapi::group_local_memory_for_overwrite is used to allocate
    group-local memory at the none kernel functor scope of a work-group data parallel kernel. You
    may need to adjust the code.
    */
    auto& reduce_buffer =
        *sycl::ext::oneapi::group_local_memory_for_overwrite<T[max_warps * elems]>(
...
#else
    /*
    DPCT1007:25: Migration of cooperative_groups::thread_block_tile::meta_group_size is not
    supported.
    */
...
DS_D_INLINE IdxReduceResult idx_reduce(sycl::group<3>& tb,
                                       sycl::sub_group& warp,
                                       float val,
                                       int idx)
{
