...
/*
DPCT1110:13: The total declared local variable size in device function fused_ln exceeds 128 bytes
...
#pragma unRoll
    ...
    /*
    DPCT1013:27: The rounding mode could not be specified and the generated code may have different
...
/*
DPCT1049:14: The work-group size passed to the SYCL kernel may exceed the limit. To get the device
...
/*
DPCT1110:15: The total declared local variable size in device function fused_residual_ln exceeds 128
...
#pragma unRoll
    ...
    const float variance = mean_diff / elems_per_row;
    /*
    DPCT1013:28: The rounding mode could not be specified and the generated code may have different
    accuracy than the original code. Verify the correctness. SYCL math built-in function rounding
    mode is aligned with OpenCL C 1.2 standard.
    */
    const float denom = sycl::rsqrt(variance + epsilon);
...
/*
DPCT1049:16: The work-group size passed to the SYCL kernel may exceed the limit. To get the device
limit, query info::device::max_work_group_size. Adjust the work-group size if needed.
*/
#define LAUNCH_FUSED_RES_LN(unRollFactor, threadsPerGroup, maxThreads)                            \
  {                                                                                               \
    ...
    stream->submit([&](sycl::handler& cgh) {                                                      \
      ...
      const T* beta_ct5 = beta;                                                                   \
      auto epsilon_ct6 = epsilon;                                                                 \
      auto elems_per_row_ct7 = elems_per_row;                                                     \
                                                                                                  \
...
#define INSTANTIATE_RES_LN(T)              \
    template void launch_fused_post_ln<T>( \
        T*, const T*, const T*, const T*, const T*, float, int, int, dpct::queue_ptr);

#define INSTANTIATE_PRE_LN_RES(T)         \
    template void launch_fused_pre_ln<T>( \
        T*, T*, const T*, const T*, const T*, const T*, float, int, int, dpct::queue_ptr);
